...
if ($email === false || !in_array($method, $methods, true) || !in_array($currency, ['USD', 'NGN'], true) || !$confirmed) {
...
$name = '';
$phone = '';
$applicationsHandle = @fopen($applicationsFile, 'rb');
if ($applicationsHandle !== false) {
    while (($application = fgetcsv($applicationsHandle)) !== false) {
        if (isset($application[1], $application[2], $application[3]) && strcasecmp($application[2], (string) $email) === 0) {
            $name = $application[1];
            $phone = $application[3];
    ...
    fclose($applicationsHandle);
}
if (!$applicationFound) reply(403, false, 'We could not match that email to a coding programme application. Please use the same email you applied with or contact Femi.');
